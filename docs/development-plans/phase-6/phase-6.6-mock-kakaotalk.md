...
### ⚠️ MVP 정책: 공개 페이지 (인증 불필요)

**주의**: MVP 단계에서는 이 페이지가 **인증 없이 접근 가능한 공개 페이지**입니다.

- **접근 권한**: 누구나 접근 가능 (로그인 불필요)
- **표시 내용**: 모든 환자 전화번호 + 발송된 메시지 전체 가시화
- **메시지 보관**: 영구 보관 (삭제 기능 없음, 스크롤 가능한 단일 채팅창)
- **실서비스 적용 시**:
  - 관리자 인증 필요
  - 환자 전화번호 마스킹 (010-****-5678)
  - 메시지 자동 삭제 또는 보관 기간 제한
